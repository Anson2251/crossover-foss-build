    ...
    microbrew() {
        ...
        if test "$2" = "build" && command -v brew >/dev/null 2>&1 && brew list "$1" &>/dev/null; then
            ...
            if test ! -d "$brew_prefix"; then
                for try_dir in /opt/homebrew/opt/"$1" /usr/local/opt/"$1"; do
                    if test -d "$try_dir"; then brew_prefix="$try_dir"; break; fi
                done
            fi
            if test -z "$brew_prefix" || test ! -d "$brew_prefix"; then
                brew_prefix="$(brew --cellar "$1" 2>/dev/null)"
            fi
            if test -n "$brew_ver" && test -d "$brew_prefix"; then
                mkdir -p "$brew_dir/$1/$brew_ver"
                for td in bin lib include share; do
                    if test -d "$brew_prefix/$td"; then
                        ln -s "$brew_prefix/$td" "$brew_dir/$1/$brew_ver/$td"
                        linkmerge "$brew_dir/$1/$brew_ver/$td" "$deps_dir/$td" "../$brew_dir_name/$1/$brew_ver/$td"
                    fi
                done
                rm -f "$deps_dir/opt/$1" && ln -s "../$brew_dir_name/$1/$brew_ver" "$deps_dir/opt/$1"
                return
            fi
        fi
...
build_wine() {
    ...
    export CFLAGS="-O2 -pipe -Wno-deprecated-declarations -Wno-incompatible-pointer-types${CFLAGS:+ $CFLAGS}"
    export LDFLAGS="-Wl,-ld_classic -Wl,-dead_strip_dylibs -Wl,-headerpad_max_install_names -Wl,-rpath,$ld_rel_name/../..${LDFLAGS:+ $LDFLAGS}"
